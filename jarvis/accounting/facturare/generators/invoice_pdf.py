...
class InvoicePdfRenderer:
    ...
    def render_one(self, c: canvas.Canvas, inv_no: int, line: OrderLine):
        ...
        if is_storno and line.storno_description:
            desc_lines.append("STORNO ADVANCE")
            desc_lines.append(f"INVOICE {line.storno_description}")
    ...
    def render_storno_group(self, c: canvas.Canvas, inv_no: int, lines: list[OrderLine]):
        """Draw a storno invoice with multiple item rows (same comanda)."""
        cfg = self.cfg
        W, H = A4
        LM = 18 * mm
        RM = W - 18 * mm
        ...
        currency = cfg.fx.currency
        for txt, x in [(currency, LM + 115 * mm), (currency, LM + 140 * mm), (currency, LM + 160 * mm)]:
            c.drawString(x, y - 17 * mm, txt)
        ...
        c.setFont("Helvetica-Bold", 11)
        c.drawString(LM, y, "PRICE")
        c.drawRightString(RM, y, f"{fmt_eur(grand_total)} {currency}")
    ...
    @staticmethod
    def _group_storno_lines(lines: list[OrderLine]) -> list[list[OrderLine]]:
        """Group storno lines by comanda, keep non-storno as single-item groups."""
        groups: list[list[OrderLine]] = []
        storno_buckets: dict[int, list[OrderLine]] = defaultdict(list)

        for line in lines:
            if line.qty < 0:
                storno_buckets[line.comanda].append(line)
            else:
                groups.append([line])

        groups.extend(storno_buckets.values())
        return groups
