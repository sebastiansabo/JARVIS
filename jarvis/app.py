...
# Tag Group endpoints (admin only)
@app.route('/api/tag-groups', methods=['GET'])
@login_required
def api_get_tag_groups():
    active_only = request.args.get('active_only', 'true').lower() != 'false'
    groups = get_tag_groups(active_only=active_only)
...
@app.route('/api/tag-groups/<int:group_id>', methods=['PUT'])
@login_required
def api_update_tag_group(group_id):
    ...
    data = request.get_json()
    try:
        updated = update_tag_group(group_id, **{k: v for k, v in data.items() if k in ('name', 'description', 'color', 'sort_order', 'is_active')})
        if updated:
            return jsonify({'success': True})
...
def api_get_tags():
    ...
    tags = get_tags(current_user.id, group_id=group_id)
    return jsonify(tags)
...
def api_update_tag(tag_id):
    ...
    data = request.get_json()
    try:
        updated = update_tag(tag_id, **{k: v for k, v in data.items() if k in ('name', 'group_id', 'color', 'icon', 'sort_order', 'is_active')})
        if updated:
            return jsonify({'success': True})
...
def api_get_entity_tags():
    ...
    tags = get_entity_tags(entity_type, entity_id, current_user.id)
    return jsonify(tags)


@app.route('/api/entity-tags/bulk', methods=['GET'])
@login_required
def api_get_entity_tags_bulk():
    ...
    if not entity_ids:
        return jsonify({})
    tags_map = get_entities_tags_bulk(entity_type, entity_ids, current_user.id)
    # Convert int keys to string keys for JSON
    return jsonify({str(k): v for k, v in tags_map.items()})


@app.route('/api/entity-tags', methods=['POST'])
@login_required
def api_add_entity_tag():
    ...
    # Verify tag is visible to user
    tag = get_tag(tag_id)
    if not tag or (not tag['is_global'] and tag['created_by'] != current_user.id):
        return jsonify({'success': False, 'error': 'Tag not found'}), 404
    added = add_entity_tag(tag_id, entity_type, entity_id, current_user.id)
    return jsonify({'success': True, 'added': added})


@app.route('/api/entity-tags', methods=['DELETE'])
@login_required
def api_remove_entity_tag():
    ...
    if not tag_id or not entity_id:
        return jsonify({'success': False, 'error': 'tag_id and entity_id are required'}), 400
    removed = remove_entity_tag(tag_id, entity_type, entity_id)
    return jsonify({'success': True, 'removed': removed})


@app.route('/api/entity-tags/bulk', methods=['POST'])
@login_required
def api_bulk_entity_tags():
    ...
    if action == 'remove':
        count = bulk_remove_entity_tags(tag_id, entity_type, entity_ids)
    else:
        # Verify tag is visible to user
        tag = get_tag(tag_id)
        if not tag or (not tag['is_global'] and tag['created_by'] != current_user.id):
            return jsonify({'success': False, 'error': 'Tag not found'}), 404
        count = bulk_add_entity_tags(tag_id, entity_type, entity_ids, current_user.id)
