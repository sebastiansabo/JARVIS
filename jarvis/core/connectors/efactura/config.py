...
This is a core connector - configuration is shared across all JARVIS sections.
"""
...
class ANAFConfig:
    ...
    # OAuth2 endpoints (for token-based authentication)
    OAUTH_BASE_URL: str = "https://logincert.anaf.ro/anaf-oauth2/v1"
    OAUTH_AUTHORIZE_URL: str = "https://logincert.anaf.ro/anaf-oauth2/v1/authorize"
    OAUTH_TOKEN_URL: str = "https://logincert.anaf.ro/anaf-oauth2/v1/token"
    OAUTH_REVOKE_URL: str = "https://logincert.anaf.ro/anaf-oauth2/v1/revoke"

    # OAuth settings
    OAUTH_SCOPE: str = "efactura"
    OAUTH_TOKEN_EXPIRY_BUFFER: int = 300  # Refresh 5 mins before expiry
    # Per ANAF OAuth documentation:
    # - Access Token JWT validity: 90 days (129,600 minutes)
    # - Refresh Token JWT validity: 365 days (525,600 minutes)
    OAUTH_ACCESS_TOKEN_VALIDITY_SECONDS: int = 7776000  # 90 days
    OAUTH_REFRESH_TOKEN_VALIDITY_SECONDS: int = 31536000  # 365 days

    # Rate limiting (per ANAF OAuth documentation: 1000 requests/minute)
    MAX_REQUESTS_PER_MINUTE: int = 1000
    RATE_LIMIT_BUFFER: int = 50  # Reserve buffer from max
...
UI_CONFIG = {
    ...
    'icon': 'file-invoice',
    'section': 'connectors',  # Core connector section
    'fields': [
        {
            'key': 'environment',
            'label': 'Environment',
