...
import { BarChart3, ClipboardCheck, Clock, Download, FileCheck, FileSpreadsheet, Fingerprint, LayoutDashboard, List, Pencil, Plus, Users, CalendarClock, ShieldCheck } from 'lucide-react'
...
const ConsentComplianceTab = lazy(() => import('./ConsentComplianceTab'))
...
export default function Hr() {
  ...
  const isConsentsPage = useMatch('/app/hr/consents')
  ...
  const canViewConsents = authLoading || !user ? true : (user?.can_access_hr ?? false)
  ...
  const tabs = useMemo(() => {
    ...
    if (canViewConsents) {
      t.push({ to: '/app/hr/consents', label: 'Acorduri', icon: ClipboardCheck })
    }
    return t
  }, [canViewTimesheets, canViewEmployees, canViewLeavePermits, canViewLeaves, canViewTimeBank, canViewCourses, canViewReports, canViewVerification, canViewConsents])
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <PageHeader
        title={
          isBonusesPage ? 'Bonuses' : isTimesheetsPage ? 'Timesheets' : isEmployeesPage ? 'Employees' : isLeavePermitsPage ? 'Bilete' : isLeavesPage ? 'Leaves (CO Balance)' : isTimeBankPage ? 'Bank' : isCoursesPage ? 'Cursuri' : isReportsPage ? 'Reports' : isVerificationPage ? 'Verification' : isConsentsPage ? 'Acorduri' : 'Pontaje'
        }
        breadcrumbs={[
          { label: 'HR', href: '/app/hr/employees' },
          ...(isBonusesPage ? [{ label: 'Bonuses' }] : isTimesheetsPage ? [{ label: 'Timesheets' }] : isEmployeesPage ? [{ label: 'Employees' }] : isLeavePermitsPage ? [{ label: 'Bilete' }] : isLeavesPage ? [{ label: 'Leaves' }] : isTimeBankPage ? [{ label: 'Bank' }] : isCoursesPage ? [{ label: 'Cursuri' }] : isReportsPage ? [{ label: 'Reports' }] : isVerificationPage ? [{ label: 'Verification' }] : isConsentsPage ? [{ label: 'Acorduri' }] : [{ label: 'Pontaje' }]),
        ]}
        search={
          <SearchInput
            value={search}
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            {!isMobile && !isBonusesPage && tabs.length > 1 && (
              <Tabs value={isConsentsPage ? 'consents' : isVerificationPage ? 'verification' : isReportsPage ? 'reports' : isTimeBankPage ? 'time-bank' : isLeavesPage ? 'leaves' : isLeavePermitsPage ? 'leave-permits' : isEmployeesPage ? 'employees' : isTimesheetsPage ? 'timesheets' : 'pontaje'} onValueChange={(v) => navigate(`/app/hr/${v}`)}>
      ...
      <Suspense fallback={<TabLoader />}>
        <Routes>
          ...
          {canViewConsents && <Route path="consents" element={<ConsentComplianceTab />} />}
        </Routes>
      </Suspense>
    </div>
  )
