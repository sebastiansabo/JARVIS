...
def test_cancel_route_returns_service_status(client, monkeypatch, login_as):
    ...
    monkeypatch.setattr(lpa, 'cancel_leave_permit', lambda sid, uid: {'status': 'cancelled'})
    login_as(user_id=9)
    r = client.post('/connecteam/api/submissions/leave-permit/42/cancel')
...
def test_cancel_route_passes_submission_id_and_current_user(client, monkeypatch, login_as):
    from core.connectors.connecteam.services import leave_permit_actions as lpa
    captured = {}
    def fake_cancel(sid, uid):
        captured['args'] = (sid, uid)
        return {'status': 'cancellation_pending'}
    monkeypatch.setattr(lpa, 'cancel_leave_permit', fake_cancel)
    login_as(user_id=9)
    r = client.post('/connecteam/api/submissions/leave-permit/42/cancel')
    assert r.status_code == 200
    assert captured['args'] == (42, 9)
