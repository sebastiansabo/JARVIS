"""Real-DB probe for test-drive booking schema integration tests. localhost/defaultdb only.
...
real DB. test_td_booking_schema.py is DB-backed (it exercises real SQL against
information_schema/pg_indexes to verify the mkt_td_* booking schema), so it
...
def _probe_real_db():
    try:
        # Ensure `database` is imported at least once under the still-mocked
        # psycopg2 (installed by jarvis/conftest.py) before any real-driver
        # rebind below. `database.py` runs init_db() as a side effect of its
        # own module-level import; against the mock that's a harmless no-op,
        # but done for the first time straight against a real connection it
        # can fail on unrelated, pre-existing schema drift in this local DB
        # (e.g. a CREATE OR REPLACE VIEW in schema_hr.py). This mirrors what
        # already happens naturally when this test module runs as part of
        # the full jarvis suite (some earlier-collected module imports
        # `database` under mock first); doing it explicitly here makes the
        # probe self-sufficient when this test file is run standalone too.
...
def require_real_db():
    ...
    if not REAL_DB_AVAILABLE:
        pytest.skip(
            ...
            'mocked) — skipping test-drive booking schema DB-backed test'
        )
